...
-- This function does not simplify properly in every case.
-- For example: In the case of (Const 0 :*: Const 5 :*: Const 7) it fails and results in Const 0 :*: Const 7. This is because it sees Const 0 :*: Const 7 as the function to simplify and does not simplify the rest.
-- The function works when we write it in this way: (Const 0 :*: (Const 5 :*: Const 7)) to force it to be evaluated as two different parts.
